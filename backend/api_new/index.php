...
header('Access-Control-Allow-Headers: Content-Type, Authorization, X-RP-Lang, Accept-Language');
...
try {
    ...
    if ($segments[0] === 'admin' && ($segments[1] ?? '') === 'support') {
        $controller = new SupportController();
        if ($method === 'GET') $controller->adminIndex();
        elseif ($method === 'PUT' && isset($segments[2])) {
            // /admin/support/{id} -> mark as resolved
            $id = (int)$segments[2];
