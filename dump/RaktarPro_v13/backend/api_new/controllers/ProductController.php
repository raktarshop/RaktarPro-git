...
class ProductController {
     ...
     * Kosárhoz: termékek lekérése ID lista alapján (PUBLIC)
     * Body: { ids: [1,2,3] }
     */
    public function byIds(): void {
        try {
            $data = Validator::getJsonInput();
            $ids = $data['ids'] ?? [];

            if (!is_array($ids)) {
                Response::error('Érvénytelen kérés: ids tömb szükséges', 400);
            }

            // csak pozitív egész ID-k
            $ids = array_values(array_unique(array_filter(array_map('intval', $ids), fn($x) => $x > 0)));

            $lang = $this->detectLang();
            $products = $this->productService->getProductsByIds($ids, $lang);
