    ...
    tbody.querySelectorAll("button[data-act='details']").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (body) {
            body.innerHTML = `
              <div class="mb-2"><b>Rendelés #${escapeHtml(id)}</b></div>
              <div class="mb-1">Státusz: ${escapeHtml(data?.status || "")}</div>
              <div class="mb-1">Fizetés: ${escapeHtml(data?.payment_method || "Utánvét")}</div>
              <div class="mb-3">Összeg: ${escapeHtml(formatMoney(data?.total_amount ?? data?.total ?? 0))}</div>
              <div class="fw-bold mb-2">Tételek</div>
