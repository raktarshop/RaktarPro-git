...
// Admin pill: "Admin: Név"
function updateAdminNamePill() {
  const pill = document.getElementById('adminNamePill');
  if (!pill) return;

  const user = getUser();
  const ok = Boolean(getToken()) && isAdminUser(user);
  if (!ok) {
    pill.textContent = 'Admin';
    return;
  }

  const name = (user?.full_name || user?.name || user?.username || user?.email || '').toString().trim();
  pill.textContent = name ? `Admin: ${name}` : 'Admin';
}
